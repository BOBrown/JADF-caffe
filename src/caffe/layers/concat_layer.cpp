...
void ConcatLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 1; i < bottom.size(); ++i) {
    ...
    for (int j = 0; j < num_axes; ++j) {
      ...
      //LOG(INFO) << "top_shape = " << top_shape[0] <<top_shape[1]<<top_shape[2]<<top_shape[3] << "num_axes = " << num_axes << "bottom.size() = " << bottom.size() << " j = " << j << " concat_axis_ = " << concat_axis_ << " i = " << i << "; top_shape[j]=" << top_shape[j] << " bottom[i]->shape(j) = " << bottom[i]->shape(j) ; by holobo
      CHECK_EQ(top_shape[j], bottom[i]->shape(j)) // by holobo
  ...
  CHECK_EQ(bottom_count_sum, top[0]->count()); //by holobo
  if (bottom.size() == 1) {
